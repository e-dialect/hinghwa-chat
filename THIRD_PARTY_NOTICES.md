...
- Upstream license: GNU Affero General Public License version 3
  (`AGPL-3.0-only`); the upstream and repository `LICENSE` texts are
  byte-for-byte identical at the audited revision.
...
版权与 GNU AGPL 第 3 版且仅该版本（`AGPL-3.0-only`）。收录这些内容不代表
